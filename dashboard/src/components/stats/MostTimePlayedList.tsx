...
import type { TrackStats } from "@/types/api";

interface MostTimePlayedListProps {
  data: TrackStats[];
  loading: boolean;
}
...
// Get source icon based on track source (keeping brand colors)
function getSourceIcon(source: string | undefined | null): React.ReactNode {
...
// Get text color based on track source
function getSourceTextColor(source: string | undefined | null): string {
  const normalizedSource = (source || "").toLowerCase();
  ...
  // SoundCloud and other non-Spotify sources get orange
  return "text-orange-600";
}

export function MostTimePlayedList({ data, loading }: MostTimePlayedListProps) {
  if (!loading && (!data || data.length === 0)) {
    return (
      <StatsListCard title="Most time played" loading={false}>
  ...
  return (
    <StatsListCard title="Most time played" loading={loading}>
            ...
            <div className="flex-1 min-w-0">
              {track.url ? (
                <a
                  ...
                  className={`text-sm truncate hover:underline ${getSourceTextColor(track.source)}`}
                ...
                <p
                  className={`text-sm truncate ${getSourceTextColor(track.source)}`}
                  title={track.name || "Unknown Track"}
                >
                  {track.name || "Unknown Track"}
                </p>
